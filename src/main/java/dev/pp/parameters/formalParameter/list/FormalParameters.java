...
import dev.pp.parameters.type.validator.DataValidator;
import dev.pp.parameters.type.validator.DataValidatorException;
...
public class FormalParameters<T> {
    ...
    public FormalParameters ( @Nullable DataValidator<List<Parameter<T>>> validator, boolean maintainOrder ) {

        if ( maintainOrder ) {
            this.allNamesMap = new LinkedHashMap<> ();
        } else {
    ...
    private void checkNotExists ( @NotNull String name ) {
        ...
        if ( containsName ( name ) )
            throw new RuntimeException ( "Parameter '" + name + "' exists already." );
    }
}
